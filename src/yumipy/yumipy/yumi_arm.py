...
class _YuMiEthernet(Process):
    ...
    def run(self):
        ...
        if self._debug:
            logging.info(
                "In DEBUG mode. Messages will NOT be sent over socket.")
        else:
            self._reset_socket()
...
class YuMiArm:
    ...
    def _request(self, req, wait_for_res, timeout=None):
        ...
        if wait_for_res:
            try:
                res = self._res_q.get(
                    block=True, timeout=self._process_timeout)
    ...
    def move_circular(self, center_pose, target_pose, wait_for_res=True):
        ...
        req_set_circ_point = YuMiArm._construct_req(
            'set_circ_point', body_set_circ_point)
    ...
    def buffer_add_all(self, pose_list, wait_for_res=True):
        ...
        YuMiCommException
            If communication times out or socket error.
        '''
        ress = [self.buffer_add_single(pose, wait_for_res)
    ...
    def close_gripper(self, force=YMC.MAX_GRIPPER_FORCE, width=0., no_wait=False,
        ...
        if force < 0 or force > YMC.MAX_GRIPPER_FORCE:
            raise ValueError("Gripper force can only be between {} and {}. Got {}.".format(
                0, YMC.MAX_GRIPPER_FORCE, force))
        if width < 0 or width > YMC > MAX_GRIPPER_WIDTH:
            raise ValueError("Gripper width can only be between {} and {}. Got {}.".format(
                0, YMC.MAX_GRIPPER_WIDTH, width))

        width = METERS_TO_MM * width
        body = YuMiArm._iter_to_str(
            '{0:.1f}', [force, width] + ([0] if no_wait else []))
    ...
    def calibrate_gripper(self, max_speed=None, hold_force=None, phys_limit=None, wait_for_res=True):
        ...
        if None in (max_speed, hold_force, phys_limit):
            ...
            body = self._iter_to_str(
                '{:.1f}', [data['max_speed'], data['hold_force'], data['phys_limit']])
        ...
        *on getting the name*
        ...
        if name in YuMiArm.__dict__:
            def handle_remote_call(*args, **kwargs):
                ...
                try:
                    response = arm(pickle.dumps(name), pickle.dumps(
                        args), pickle.dumps(kwargs))
            ...
            arm = rospy.ServiceProxy(self.arm_service, ROSYumiArm)
            try:
                response = arm(pickle.dumps('__getattribute__'),
                               pickle.dumps(name), pickle.dumps(None))
            except rospy.ServiceException as e:
                raise RuntimeError(
    ...
    def YuMiArm(arm_type, name, ros_namespace=None):
        ...
        if arm_type == 'local':
            ...
            if ROS_ENABLED:
                ...
                raise RuntimeError(
                    "Remote YuMiArm is not enabled because yumipy is not installed as a catkin package")
        else:
