...
class YuMiRobot:
    ...
    def __init__(self, ip=YMC.IP, port_l=YMC.PORTS["left"]["server"], port_r=YMC.PORTS["right"]["server"], tcp=YMC.TCP_DEFAULT_GRIPPER,
        ...
        if include_left:
            if arm_type == 'local':
                ...
                self.left = YuMiArmFactory.YuMiArm(
                    'remote', 'left', ros_namespace)
            else:
                raise RuntimeError(
                    "arm_type {0} for YuMiArm is not a valid arm type".format(arm_type))
        ...
        if include_right:
            if arm_type == 'local':
                ...
                self.right = YuMiArmFactory.YuMiArm(
                    'remote', 'right', ros_namespace)
            else:
                raise RuntimeError(
                    "arm_type {0} for YuMiArm is not a valid arm type".format(arm_type))
    ...
    def goto_state_sync(self, left_state, right_state):
        ...
        if len(self._arms) != 2:
            raise Exception(
                "Cannot goto state sync when not both arms are included!")
    ...
    def goto_pose_sync(self, left_pose, right_pose):
        ...
        if len(self._arms) != 2:
            raise Exception(
                "Cannot goto pose sync when not both arms are included!")
