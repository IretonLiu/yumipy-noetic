...
class _YuMiSubscriberEthernet(Process):
    ...
    def _stop(self):
        logging.debug(
            "Shutting down yumi subscriber ethernet interface for {0}".format(self._name))
        self._socket.close()

    def _reset_socket(self):
        logging.debug('Opening socket on {0}:{1} for subscription'.format(
            self._ip, self._port))
...
class _YuMiArmSubscriber:
    ...
    def _get_data(self, name, timestamp=True):
        if name not in self.includes:
            raise ValueError(
                "Subscriber wasn't instantiated to include {}!".format(name))
...
class YuMiSubscriber:
    ...
    def stop(self):
        ...
        if not self._started:
            raise Exception(
                "Cannot stop a YuMiSubscriber if it hasn't started yet!")
    ...
    def reset_time(self):
        if not self._started:
            raise Exception(
                "Can only reset time if YuMiSubscriber has started!")
